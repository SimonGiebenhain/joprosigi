...
def preprocess_dataset(dataset):
    ...
    dataset['num_words_title'] = ztransform(dataset.num_words_title.values)
    dataset['num_words_description'] = ztransform(np.log1p(dataset.num_words_description))
    dataset['num_unique_words_title'] = ztransform(dataset.num_unique_words_title.values)
    dataset['num_unique_words_description'] = ztransform(np.log1p(dataset.num_unique_words_description.values))
    dataset['words_vs_unique_title'] = ztransform(dataset.words_vs_unique_title.values)
    dataset['words_vs_unique_description'] = ztransform(dataset.words_vs_unique_description.values)
    dataset['num_desc_punct'] = ztransform(dataset.num_desc_punct.values)
...
def RNN_model():
    ...
    # model
    model = Model([seq_title_description, region, city, wday, user_type, no_price, category_name, parent_category_name,
