...
from keras.layers import CuDNNGRU, GRU, Bidirectional, GlobalAveragePooling1D, GlobalMaxPooling1D, BatchNormalization, Conv1D, MaxPooling1D, Flatten
...
def preprocess_dataset(dataset):
    ...
    del dataset['avg_days', 'avg_times', 'n_times']
    gc.collect()
...
def get_keras_data(dataset, tokenizer):
    ...
    X = {
        ...
        'avg_days_up_cat': dataset.avg_days_up_cat.values,
        'avg_times_up_cat': dataset.avg_times_up_cat.values,
        'n_cat_items': dataset.n_cat_items
    }
    return X
...
def prep_df():
    ...
    train.to_csv('../data/new_rnn_train_nz.csv')
    del train
    gc.collect()
    test = all[train_size:]
    test.to_csv('../data/new_rnn_test_nz.csv')
    del test
    del all
    gc.collect()
    np.save('../data/new_rnn_y_train_nz.npy', y_train)
...
def RNN_model():
    ...
    x = Conv1D(64, kernel_size=3, padding="valid", kernel_initializer="glorot_uniform", name='nlp_conv')(x)
    ...
    cat_net = Dense(256)(cat_net)
    cat_net = Dropout(0.5)(add_common_layers(cat_net))
    cat_net = Dense(128)(cat_net)
    cat_net = Dropout(0.5)(add_common_layers(cat_net))
    cat_net = Dense(64)(cat_net)
    cat_net = Dropout(0.5)(add_common_layers(cat_net))
    cont_net = concatenate([cat_net,
                            ...
                            avg_cat_times,
                            n_cat_items])
    cont_net = Dense(128)(cont_net)
    cont_net = Dropout(0.5)(add_common_layers(cont_net))
    cont_net = Dense(64)(cont_net)
    cont_net = Dropout(0.5)(add_common_layers(cont_net))
    cont_net = Dense(32)(cont_net)
    cont_net = Dropout(0.2)(cont_net)
    main_net = concatenate([x, cat_net, cont_net])
    main_net = Dense(256)(main_net)
    main_net = Dropout(0.5)(add_common_layers(main_net))
    main_net = Dense(128)(main_net)
    main_net = Dropout(0.3)(add_common_layers(main_net))
    # output
    output = Dense(1, activation="sigmoid")(main_net)

    # model
    model = Model([seq_title_description, region, city, day, wday, user_type, no_price, category_name, parent_category_name,
...
def train_only():
    train = pd.read_csv('../data/new_rnn_train_nz.csv')
    y_train = np.load('../data/new_rnn_y_train_nz.npy')
    ...
    K.set_value(modelRNN.optimizer.decay, lr_decay)
    K.set_value(modelRNN.optimizer.clipnorm, 1)

    X_train = get_keras_data(train, tokenizer)

    # Fit the NN Modelj,
    checkpt = ModelCheckpoint(monitor="val_loss", mode="auto", filepath='weights/rnn_model_baseline_weights_noconv_2.hdf5',
                              verbose=1,
                              save_best_only=True)
    ...
    modelRNN.fit(X_train, y_train,
                 ...
                 validation_split=0.33,callbacks=[checkpt],
                 shuffle=True,
                 verbose=2)
    # Predict test set for Kth Fold
    modelRNN.save_weights('./weights/new_rnn_with_add_feat_nz.hdf5')
